...
public class Kollections {
    ...
    private static <T extends Comparable<T>> int binarySearch(List<T> sortedList, T element, int from, int to) {
        ...
        if(element.compareTo(sortedList.get(mid)) < 0){
            result = binarySearch(sortedList, element, from, mid);
        } else if(element.compareTo(sortedList.get(mid)) > 0){
            result = binarySearch(sortedList, element, mid, from);
        } else {
            return mid;
        }
        return result;
